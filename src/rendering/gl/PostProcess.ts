...
class PostProcess extends ShaderProgram {
	...
	unifD: WebGLUniformLocation;
	unifVel: WebGLUniformLocation;
	name: string;
	...
	constructor(fragProg: Shader, tag: string = "default") {
		...
		this.unifD = gl.getUniformLocation(this.prog, "u_gb0");
		this.unifVel = gl.getUniformLocation(this.prog, "u_gb1");
		this.use();
		this.name = tag;

		// bind texture unit 0 to this location
		gl.uniform1i(this.unifFrame, 0); // gl.TEXTURE0
		gl.uniform1i(this.unifD, 3);
		gl.uniform1i(this.unifVel, 4);
